...
class TransformersSeqClassifierHandler(BaseHandler, ABC):
    ...
    def preprocess(self, requests):
        ...
        for idx, data in enumerate(requests):
            ...
            #    print("<<<<<<<<<<<<<<<<<<,Inputs, type: ", input_text, type(input_text))
            ...
            conversion_list = input_ids.squeeze(dim=0).tolist()
            print("Conversion list: ", conversion_list)
            non_zeros = [x for x in conversion_list if x != 0]
            print("Non zeros: ", non_zeros)
            print("Number of non zeros", len(non_zeros))

            am_1 = torch.ones(len(non_zeros))
            am_0 = torch.zeros(int(max_length) - len(non_zeros))
            attention_mask = torch.cat((am_1, am_0), 0)
            
            #input_ids = input_ids.unsqueeze(dim=0)
            attention_mask = attention_mask.unsqueeze(dim=0)
            print("Generated input ids and attention mask: ", input_ids, input_ids.shape, attention_mask, attention_mask.shape)
        ...
        print("Returning input ids batch: ", input_ids_batch)
        print("Returning attention mask batch: ", attention_mask_batch)
    ...
    def inference(self, input_batch):
        ...
        if self.setup_config["mode"] == "sequence_classification":
            ...
            for i in range(num_rows):
                output = predictions[0][i].unsqueeze(0)
                print("Derived output in inference:", output)
    ...
    def postprocess(self, inference_output):
        ...
        return [inference_output.argmax(1).flatten().tolist()]
